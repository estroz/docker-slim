...
func GetContainerBuildOptions(ctx *cli.Context) (*config.ContainerBuildOptions, error) {
	...
	for _, rba := range rawBuildArgs {
		...
		case 2:
			...
			cbo.BuildArgs = append(cbo.BuildArgs, config.CBOBuildArg{
				Name:  parts[0],
				Value: parts[1],
			})
		case 1:
...
func GetFileMatcherConfig(ctx *cli.Context, volumeMounts map[string]config.VolumeMount) (cfg *config.FileMatcherConfig, err error) {
	cfg, err = config.NewFileMatcherConfig(ctx, ctx.String(commands.FlagSlimKeep))
	if err != nil {
		return nil, err
	}
	if cfg.Matcher != nil {
		...
		if len(badFlags) != 0 {
			return nil, fmt.Errorf("cannot set legacy flags with .slimkeep present: %s", strings.Join(badFlags, ", "))
		}
		return cfg, nil
	}
...
// legacyFileMatcherConfigFlags holds all flags that can be captured by a .slimkeep.
